...
def parameterizing_function(navigation, breaking, crafting, prev_width, prev_height, prev_length):
	...
	return width, height, length, type_of_env
...
if __name__ == "__main__":
	...
	curriculum_params = [[{'width0': 0, 'height0': 0, 'length0':0, 'no_trees': 0, 'no_rocks': 0, 'starting_trees': 0, 'starting_rocks': 0, 'crafting_table': 0, 'navigation': 0, 'breaking': 0, 'crafting': 0, 'type_of_env0': 0}] for _ in range(beam_search_width)]
	...
	for i in range(no_of_environmets):
		...
		width, height, length, type_of_env = parameterizing_function(navigation = 0, breaking = 0, crafting = 0, prev_width = 0, prev_height = 0, prev_length = 0)
		...
		env = gym.make(env_id, map_width = width, map_height = height, map_length = length, goal_env = type_of_env)
		D = len(env.low)
	...
	for beam_no in range(beam_search_width):
		...
		curriculum_params[beam_no][0]['length0'] = length_array[elements_list[beam_no]]
	...
	curriculum_params_1 = [[{'width0': 0, 'height0': 0, 'length0':0, 'no_trees': 0, 'no_rocks': 0, 'starting_trees': 0, 'starting_rocks': 0, 'crafting_table': 0, 'navigation': 0, 'breaking': 0, 'crafting': 0, 'type_of_env0' : 0,\
	'width1': 0, 'height1': 0, 'length0':1, 'no_trees1': 0, 'no_rocks1': 0, 'starting_trees1': 0, 'starting_rocks1': 0, 'crafting_table1': 0, 'type_of_env1' : 0}] for _ in range(beam_search_width)]

	curriculum_params_2 = [[{'width0': 0, 'height0': 0,'length0':0, 'no_trees': 0, 'no_rocks': 0, 'starting_trees': 0, 'starting_rocks': 0, 'crafting_table': 0, 'navigation': 0, 'breaking': 0, 'crafting': 0, 'type_of_env0' : 0, \
	'width1': 0, 'height1': 0, 'no_trees1': 0, 'length1':0,'no_rocks1': 0, 'starting_trees1': 0, 'starting_rocks1': 0, 'crafting_table1': 0, 'type_of_env1' : 0, \
	'width2': 0, 'height2': 0, 'no_trees2': 0, 'length2':0,'no_rocks2': 0, 'starting_trees2': 0, 'starting_rocks2': 0, 'crafting_table2': 0, 'type_of_env2' : 0}] for _ in range(beam_search_width)]
	...
	for curriculum_number in range(1, curriculum_breadth):
		...
		for beam_number in range(beam_search_width):
			for environment_number in range(no_of_environmets):
				...
				if curriculum_number == 1:
					...
					prev_length = curriculum_params[beam_number][0]['length' + str(curriculum_number - 1)]
				...
				width, height, length, type_of_env = parameterizing_function(navigation = no_navigation, breaking = no_breaking, crafting = no_crafting, prev_width = prev_width, prev_height = prev_height,prev_length = prev_length)
				width_after.append(width)
				height_after.append(height)
				length_after.append(length)
		...
		for i in range(beam_search_width):
			...
			if curriculum_number == 1:
				...
				curriculum_params_1[i][0].update({'length'+str(curriculum_number) : length_after[elements_list[i]]})
			...
			if curriculum_number == 2:
				...
				curriculum_params_2[i][0].update({'length'+str(curriculum_number) : length_after[elements_list[i]]})
	...
	for final_env in range(beam_search_width):
		...
		final_status = True
		env_id = 'NovelGridworld-v1'
		env = gym.make(env_id, map_width = 8, map_height = 8, map_length = 8, goal_env = 2)
