...
env.seed(MPI.COMM_WORLD.Get_rank() + random_seed)
random.seed(MPI.COMM_WORLD.Get_rank() + random_seed)
np.random.seed(MPI.COMM_WORLD.Get_rank() + random_seed)
torch.manual_seed(MPI.COMM_WORLD.Get_rank() + random_seed)
...
if Train:
    ...
    total_ac_loss = []
    total_cr_loss = []
    for epoch in range(MAX_EPOCHS_0):
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        ...
        plt.figure()
        plt.plot(np.arange(0, MAX_EPOCHS_0), t_success_rate)
        plt.title("Success rate")
        plt.savefig("success_rate_0.png")


    ENV_NAME_1 = "PandaCurriculum2-v2"
    env = gym.make(ENV_NAME_1)
    env.seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    random.seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    np.random.seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    torch.manual_seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    agent.clear_memory(dc(env))
    t_success_rate = []
    total_ac_loss = []
    total_cr_loss = []    
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        ...
        plt.style.use('ggplot')
        plt.figure()
        plt.plot(np.arange(0, MAX_EPOCHS_1), t_success_rate)
        plt.title("Success rate")
        plt.savefig("success_rate_1.png")
        # plt.show()

    ENV_NAME_1 = "PandaCurriculum3-v2"
    env = gym.make(ENV_NAME_1)
    env.seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    random.seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    np.random.seed(MPI.COMM_WORLD.Get_rank()+random_seed)
    ...
    if MPI.COMM_WORLD.Get_rank() == 0:
        ...
        plt.style.use('ggplot')
        plt.figure()
        plt.plot(np.arange(0, MAX_EPOCHS_2), t_success_rate)
        plt.title("Success rate")
        plt.savefig("success_rate_2.png")

        experiment_file_name_success_rate = 'randomseed_' + str(random_seed) + '_success_rate'
        path_to_save_success_rate =  experiment_file_name_success_rate + '.npz'        
        np.savez_compressed(path_to_save_success_rate, curriculum_timesteps = t_success_rate)
        print("curriculum timsteps: ", curriculum_t)
        print("curriculum + final timsteps: ", global_t)
