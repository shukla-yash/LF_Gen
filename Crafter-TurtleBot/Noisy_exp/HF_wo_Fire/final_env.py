...
def CheckTrainingDoneCallback(reward_array, done_array, env):
	...
	if len(done_array) > 30:
		...
		if done_cond == True:
			if env < 3:
				if np.mean(reward_array[-40:]) > 700:
					reward_cond = True
			# else:
...
if __name__ == "__main__":
	...
	print("size final_avg_reward_array: ", final_task_completion_arr.shape)
	...
	experiment_file_name_final_task_completion_arr = 'randomseed_' + str(random_seed) + '_final_task_completion'
	path_to_save_final_task_completion_arr = log_dir + os.sep + experiment_file_name_final_task_completion_arr + '.npz'
	...
	np.savez_compressed(path_to_save_final_task_completion_arr, final_task_completion = final_task_completion_arr)
